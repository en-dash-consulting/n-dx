...
// FileStore mutations now write only to .rex/tree/ — prd.md is no longer
// regenerated by addItem/updateItem/removeItem. These tests assert on prd.md
// after writes, so they are skipped until rewritten to read via
// store.loadDocument()/parseFolderTree.
describe.skip("Store roundtrip integration", () => {
  let tmpDir: string;
  let rexDir: string;
  let store: PRDStore;
