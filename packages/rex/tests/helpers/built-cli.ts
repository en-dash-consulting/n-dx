 ...
 *
 * Some behaviour can only be tested through a real process — git launches the
 * merge driver itself, and CLI hint output is only meaningful from a real
 * argv. Those tests depend on a build step that the import graph cannot see,
 * so when the build is missing or out of date they fail somewhere unrelated to
 * the cause.
 *
 * Existence alone is not enough. A dist that predates the source fails with
 * whatever the old code did — the merge-driver suite reported
 * `NDX_CLI_NOT_INITIALIZED` against a git temp path, which names the project
 * directory, the merge, and the driver, but never the build. Diagnosing that
 * costs a full validation cycle; the fix was `pnpm build` with no source
 * change at all.
 *
...
export function requireFreshBuiltCli(overrides?: Partial<BuiltCliPaths>): string {
  ...
  if (!existsSync(cliPath)) {
    throw new Error(
      ...
      `Run \`pnpm build\` (or \`pnpm --filter @n-dx/rex run build\`) first.`,
    );
  }
