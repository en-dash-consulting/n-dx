...
const { mockExecStdout, mockUserInfo } = vi.hoisted(() => ({
...
import { resolveActor, _resetActorCacheForTests } from "../../../src/core/identity.js";
...
describe("resolveActor", () => {
  ...
  it("de-dupes concurrent calls into a single resolution", async () => {
    mockGitConfig({ "user.name": "Ada Lovelace", "user.email": "ada@example.com" });

    const [a, b] = await Promise.all([resolveActor(), resolveActor()]);

    expect(a).toBe(b);
    expect(mockExecStdout).toHaveBeenCalledTimes(2);
  });
