# PRD Analysis Core — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: PRD Analysis Core (`prd-analysis-core`)
Files: 69, Cohesion: 0.52, Coupling: 0.48
Risk: healthy (score: 0.48)
Description: The central domain hub: PRD schema definitions, the full analysis pipeline (decompose, dedupe, diff, validate), core structural transitions, and the primary CLI commands for analyzing, adding, and managing PRD items.
...
src/cli/commands/status.ts (TypeScript, 495 lines, source)
...
Incoming (other zones → this zone):
  ...
  ← fix-command: src/core/fix.ts → src/schema/index.ts; tests/unit/cli/commands/fix.test.ts → src/schema/index.ts; tests/unit/core/fix.test.ts → src/schema/index.ts
...
[observation] [info] CLI command files (add.ts, smart-add.ts, status.ts) co-located with domain logic (decompose.ts, dedupe.ts, diff.ts) mixes presentation and domain layers within a single zone, reducing cohesion clarity.
[observation] [warning] With 69 files and 25 entry points, this zone is the dependency hub of the system. High fan-in from cli, core, store, and unit zones means changes here have wide blast radius — consider treating public API surface (schema/index.ts, analyze/index.ts) as a formal contract.
[suggestion] [info] Zone "prd-analysis-core" has files across 12 directories — consider consolidating under a dedicated directory
[suggestion] [warning] 25 entry points with no public.ts means the public API surface grows by default with every new file. Add a src/public.ts barrel that explicitly controls what is exported, matching the pattern used by other packages in this monorepo.
[suggestion] [warning] consolidation-guard.ts and smart-add-duplicates.ts are runtime defensive guards against data shape violations. Tightening the schema types in src/schema/v1.ts (narrower union types, branded IDs, non-optional fields) would move these checks to compile time and eliminate the need for the runtime guards entirely.
[pattern] [critical] prd-analysis-core has zero unidirectional dependencies — all 5 zones that import it also receive imports back, making it impossible to extract or stabilize in isolation. This is the most systemic architectural risk in the codebase.
[relationship] [critical] The store → prd-analysis-core (22) and prd-analysis-core → store (8) cycle means analysis logic and persistence are mutually entangled; neither can be tested or deployed independently.
[anti-pattern] [critical] src/analyze/llm-bridge.ts embeds a direct Claude CLI subprocess adapter inside the domain hub; LLM I/O should be injectable (interface + adapter in llm-client) so analysis logic can be unit-tested without a live Claude process.
[anti-pattern] [warning] src/analyze/reshape-reason.ts is classified in cli-mcp-interface but lives in src/analyze/ and imports exclusively from other prd-analysis-core files (reason.ts, core/reshape.ts) — it is a misplaced file that belongs in prd-analysis-core.
[anti-pattern] [critical] src/schema/index.ts and src/schema/v1.ts are high-fan-in type-only modules (71 importers) co-located inside the domain hub zone — they should be extracted to a foundation layer so consumers can depend on schema types without coupling to analysis logic.
...
- [call graph] 2450 internal calls, 296 outgoing, 202 incoming (cohesion: 0.89, coupling: 0.11)
...
- **Unit Analyze/unit** (`unit-analyze/unit`): 51 files, cohesion 0.84, coupling 0.16
  - Has 4 nested sub-zone(s)
- **Unit Analyze/unit 2** (`unit-analyze/unit-2`): 3 files, cohesion 0.33, coupling 0.67
- **Unit Analyze/unit Analyze** (`unit-analyze/unit-analyze`): 4 files, cohesion 0.33, coupling 0.67
- **Unit Analyze/unit Cli** (`unit-analyze/unit-cli`): 11 files, cohesion 0.77, coupling 0.23
