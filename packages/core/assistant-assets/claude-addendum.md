...
**What the code enforces:** the MCP write tools and the bulk restructurers `reorganize`, `prune`, and `reshape` perform their read-modify-write inside `store.withTransaction`, which holds the PRD file lock across the whole span — a concurrent writer's item is no longer silently dropped by their full-document save. Their LLM analysis runs on an unlocked snapshot; accepted proposals are re-applied against a freshly loaded document under the lock. `saveDocument` itself always takes the lock, so writes cannot interleave, and a writer that cannot acquire the lock within its timeout fails loudly with an error naming the holder PID rather than proceeding. A writer that finds the lock held waits for it rather than failing fast — same-process callers queue on an in-process mutex, other processes retry — so a lock error means the holder outlasted the whole timeout. The post-mutation `syncFolderTree` step is inside the same lock and passes `knownItemIds` — the ids its own load actually saw — so it cannot re-serialize a snapshot taken before a writer that was queued behind the transaction it follows. The stale-save guard identifies items rather than timing them: a deletion candidate whose id was absent from the loaded document belongs to another writer and aborts the save. It previously compared file mtimes against a load timestamp, which false-positived because those two values come from different clocks.
