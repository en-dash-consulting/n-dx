 ...
 *
 * @param {string} primer  Raw primer file contents.
 * @returns {string | undefined}
 */
export function readPrimerFingerprint(primer) {
  const firstLine = primer.split("\n", 1)[0] ?? "";
  if (!firstLine.startsWith(PRIMER_FINGERPRINT_PREFIX)) return undefined;
 ...
 *
 * A primer is only used when the fingerprint it was stamped with matches the
 * current analysis. Existence is not freshness: `sourcevision analyze` leaves
 * the previous primer in place when a distillation is skipped, so an unchecked
 * read would hand every task in a loop a description of the repo as it stood
 * at an older commit — stated with the same confidence as a current one. A
 * stale primer is worse than none, so it is ignored rather than trusted, and
 * CONTEXT.md (regenerated by every analysis) serves instead.
 *
 * Falls back to CONTEXT.md whenever no usable primer exists: one is written
 * best-effort, so its absence is an ordinary state (an LLM-free `--lite`
 * analysis, or a distillation that was skipped) and not an error.
 *
 * @param {string} dir  Project root directory.
...
export function readContextMd(dir) {
  ...
  if (existsSync(primerPath)) {
    try {
      ...
      // An empty or stale primer is treated as absent rather than as context.
      if (content.trim() && readPrimerFingerprint(content) === sourcevisionAnalysisFingerprint(dir)) {
        return { content, source: "primer" };
      }
    } catch {
