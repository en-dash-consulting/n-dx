/**
 * Unit tests proving `callClaude` spawns the vendor CLI in the directory set
 * via `setProjectDir()`, not the calling process's cwd. Mirrors
...
import { callClaude, setLLMConfig, setProjectDir } from "../../../src/analyzers/claude-client.js";
...
describe("callClaude — cwd pass-through via setProjectDir", () => {
  ...
  it("does not override the inherited cwd when setProjectDir was never called", async () => {
    const cliDir = mkdtempSync(join(tmpdir(), "ndx-sv-cwd-bin-"));
    try {
      const cliPath = writeCwdEchoingFakeCli(cliDir);
      setLLMConfig({ vendor: "claude", claude: { cli_path: cliPath } });

      const result = await callClaude("hi");
  ...
  it("spawns the vendor CLI in the directory set by setProjectDir", async () => {
    const cliDir = mkdtempSync(join(tmpdir(), "ndx-sv-cwd-bin-"));
    const targetDir = mkdtempSync(join(tmpdir(), "ndx-sv-cwd-target-"));
    try {
      const cliPath = writeCwdEchoingFakeCli(cliDir);
      setLLMConfig({ vendor: "claude", claude: { cli_path: cliPath } });
