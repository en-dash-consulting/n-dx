...
import { mockStoreWithTracking } from "../../helpers/index.js";

describe("updateCompletedTaskStatus", () => {

  it("updates task status from pending to completed", async () => {
    const { store, getUpdatedStatus } = mockStoreWithTracking("pending");
    const run: RunRecord = {
      id: "run-1",
      taskId: "task-1",
      taskTitle: "Task 1",
  ...
  it("updates task status from in_progress to completed", async () => {
    const { store, getUpdatedStatus } = mockStoreWithTracking("in_progress");
    const run: RunRecord = {
      id: "run-1",
      taskId: "task-1",
      taskTitle: "Task 1",
  ...
  it("is idempotent when task is already completed", async () => {
    const { store, getUpdatedStatus } = mockStoreWithTracking("completed");
    const run: RunRecord = {
      id: "run-1",
      taskId: "task-1",
      taskTitle: "Task 1",
