 ...
 * and `init` side-effects are only meaningful from a real argv. Those tests
 * depend on a build step that the import graph cannot see, so when the build
 * is missing or out of date they fail somewhere unrelated to the cause.
 *
...
export function requireFreshBuiltCli(overrides?: Partial<BuiltCliPaths>): string {
  ...
  if (!existsSync(cliPath)) {
    throw new Error(
      ...
      `Run \`pnpm build\` (or \`pnpm --filter @n-dx/hench run build\`) first.`,
    );
  }
