...
export function buildReviewBrief(ctx: ReviewPromptContext): string {
  ...
  lines.push(
    ...
    "- **Stage every file you fix.** Run `git add <path>` for each file a",
    "  must-fix repair touches, as soon as the repair is done. The run's commit",
    "  takes the index as it stands; an unstaged repair is not in it, however",
    "  confidently your report says `fixed`.",
    "- **Do not commit.** The run commits after you finish; your staged fixes are",
    "  picked up by that commit. Do not run `git commit`, `git reset`,",
    "  `git checkout`, `git clean`, or anything else that rewrites history or",
    "  discards work.",
