# Message Flow Control — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Message Flow Control (`message-flow-control`)
Files: 7, Cohesion: 0.64, Coupling: 0.36
Description: Implements rate limiting, message coalescing, and throttling for optimal communication performance.
...
Outgoing (this zone → other zones):
  → request-deduplication-system: packages/web/tests/integration/request-dedup.test.ts → packages/web/src/viewer/request-dedup.ts

Incoming (other zones → this zone):
  ← project-status-ui: packages/web/src/viewer/components/status-indicators.ts → packages/web/src/viewer/message-coalescer.ts; packages/web/src/viewer/components/status-indicators.ts → packages/web/src/viewer/message-throttle.ts
  ← web-platform: packages/web/src/viewer/views/prd.ts → packages/web/src/viewer/call-rate-limiter.ts; packages/web/src/viewer/views/prd.ts → packages/web/src/viewer/message-coalescer.ts; packages/web/src/viewer/views/prd.ts → packages/web/src/viewer/message-throttle.ts

</imports>

<findings>

[suggestion] [info] Move request-dedup.test.ts to unit/viewer/ to match other flow control utility tests, or justify integration categorization with cross-zone dependencies
