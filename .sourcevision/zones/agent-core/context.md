...
Description: Core autonomous agent functionality including analysis, planning, and lifecycle management.
...
Outgoing (this zone → other zones):
  ...
  → specialized-tools: packages/hench/src/agent/lifecycle/cli-loop.ts → packages/hench/src/tools/index.ts
  → task-completion: packages/hench/src/agent/index.ts → packages/hench/src/validation/completion.ts; packages/hench/src/agent/index.ts → packages/hench/src/validation/completion.ts; packages/hench/src/agent/lifecycle/cli-loop.ts → packages/hench/src/validation/completion.ts
...
Incoming (other zones → this zone):
  ...
  ← tool-security: packages/hench/src/guard/index.ts → packages/hench/src/schema/index.ts; packages/hench/tests/unit/tools/files.test.ts → packages/hench/src/schema/v1.ts; packages/hench/tests/unit/tools/shell.test.ts → packages/hench/src/schema/v1.ts
...
[observation] [info] Well-architected agent core with strong cohesion and manageable coupling
[relationship] [warning] Heavy bidirectional coupling with cli-infrastructure (30 total imports) suggests agent logic bleeding into CLI concerns
[anti-pattern] [warning] Token budget and usage tracking logic duplicated across lifecycle files instead of centralized service
[anti-pattern] [warning] Zone mixes three distinct architectural layers: analysis (review, summary), planning (brief, context), and lifecycle (loops, token management) that should be separate zones
[suggestion] [warning] Analysis subfolder implements post-execution concerns mixed with core logic - separate into analysis-reporting zone
[suggestion] [warning] Token management files in lifecycle/ subfolder violate single responsibility - extract to dedicated resource-management zone
