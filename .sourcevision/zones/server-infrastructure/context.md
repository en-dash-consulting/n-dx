# Server Infrastructure — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Server Infrastructure (`server-infrastructure`)
Files: 69, Cohesion: 0.94, Coupling: 0.06
Risk: healthy (score: 0.06)
Description: Backend services providing MCP endpoints, cross-package gateways, and server orchestration.
...
Outgoing (this zone → other zones):
  ...
  → web-auth: packages/web/src/public.ts → packages/web/src/server/websocket.ts; packages/web/src/server/index.ts → packages/web/src/server/websocket.ts; packages/web/src/server/routes-hench.ts → packages/web/src/server/websocket.ts; packages/web/src/server/routes-rex.ts → packages/web/src/server/websocket.ts; packages/web/src/server/start.ts → packages/web/src/server/websocket.ts; packages/web/src/server/start.ts → packages/web/src/server/ws-health-tracker.ts
  → web-middleware: packages/web/src/server/routes-hench.ts → packages/web/src/server/incremental-task-usage.ts; packages/web/src/server/start.ts → packages/web/src/server/usage-cleanup-scheduler.ts
...
Incoming (other zones → this zone):
  ...
  ← web-middleware: packages/web/src/server/usage-cleanup-scheduler.ts → packages/web/src/server/rex-gateway.ts; packages/web/src/server/usage-cleanup-scheduler.ts → packages/web/src/server/rex-gateway.ts
...
[suggestion] [info] Gateway modules successfully implement documented cross-package import pattern but lack explicit interface contracts for type safety
...
- Gateway modules (domain-gateway.ts, rex-gateway.ts) implement the documented cross-package import pattern
- Aggregation cache and metrics suggest performance-conscious server design
- Clean separation between MCP protocol handling and domain logic through gateway abstraction
- Gateway pattern successfully isolates cross-package dependencies into dedicated boundary modules
- Acts as service orchestration hub for multiple smaller web zones, suggesting microservice coordination pattern
- Server zone coordinates multiple smaller web zones through hub-and-spoke pattern, centralizing service integration
- Dynamic service configuration pattern through port.ts utility and mcp-deps.ts suggests runtime service discovery capabilities
- Concurrent execution tracking via concurrent-execution-metrics.ts indicates multi-threaded or async workload management
- Gateway modules successfully implement documented cross-package import pattern but lack explicit interface contracts for type safety
