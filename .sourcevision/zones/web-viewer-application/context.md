# Web Viewer Application — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Web Viewer Application (`web-viewer-application`)
Files: 387, Cohesion: 0.98, Coupling: 0.02
Risk: healthy (score: 0.02)
Description: The full-stack web package implementation encompassing the Preact dashboard viewer, server-side entry points, schema definitions, and CLI wiring for the unified dashboard and MCP endpoints.
...
[observation] [info] Contains 59% of project files (387/651) — subdivided into 8 sub-zones
[observation] [info] High cohesion (0.98) — files are tightly interconnected
[observation] [info] Cohesion of 0.98 is the highest in the codebase — this zone is exceptionally well-bounded and its internal modules are tightly related.
[observation] [info] External coupling of 0.02 confirms the viewer application imports very little from outside its own boundary, which aligns with the gateway-only cross-package import rule.
[observation] [info] With 387 files the zone is large enough that sub-layer drift (e.g. viewer components importing directly from server modules) should be verified by boundary-check.test.ts on each refactor.
[suggestion] [info] Zone "web-viewer-application" has files across 22 directories — consider consolidating under a dedicated directory
[suggestion] [warning] The 10 entry points create an unbounded public API surface with no deprecation path: any symbol exported from any of the 10 entry point files is implicitly public. Consolidating to a single public.ts re-export barrel (matching the convention documented in CLAUDE.md for all 5 packages) would make the API surface explicit, auditable, and safely evolvable without a monorepo-wide grep.
[suggestion] [warning] rex-gateway.ts (the Rex MCP server factory) lives in web-viewer-application but is consumed exclusively by mcp-route-layer. Moving rex-gateway.ts to mcp-route-layer would co-locate the gateway with its sole consumer, eliminate one cross-zone import, and make the composition-root zone self-contained — a before/after improvement with no source code changes required, only a zone pin update.
[pattern] [warning] packages/web/src/landing/landing.ts and packages/web/tests/unit/landing/landing.test.ts are algorithmically placed in monorepo-root but are web viewer UI files; pinning them to web-viewer-application would remove misclassified files from the root zone.
[relationship] [info] web-viewer-application is the middle tier in a documented three-layer stack (mcp-route-layer → web-viewer-application → web-shared-foundation); its bidirectional coupling with mcp-route-layer is bounded by this stack position and should not be treated as a layering violation.
[anti-pattern] [warning] The boundary-check.test.ts file is the sole enforcement mechanism for the 4 documented internal sub-layer boundaries within web-viewer-application. If a new import path violates a sub-layer rule but is not covered by that test, the violation will accumulate silently — zone metrics will remain green because all violations stay intra-zone. Consider adding explicit import-direction assertions for each sub-layer pair (server→viewer, viewer→messaging, messaging→shared) to make coverage gaps visible.
[anti-pattern] [warning] packages/web/src/server/start.ts is algorithmically placed in web-viewer-application but is the actual server composition root — it imports handleMcpRoute from routes-mcp.ts (mcp-route-layer), creating an upward zone crossing. Pinning start.ts to mcp-route-layer alongside the handler it wires would eliminate the upward import, restore the pure-sink property of the composition-root zone, and make mcp-route-layer's zone membership structurally accurate.
...
- Contains 59% of project files (387/651) — subdivided into 8 sub-zones
- 10 entry points — wide API surface, consider consolidating exports
- Exemplary cohesion (0.98) and minimal coupling (0.02) confirm strong internal boundary enforcement across the 387-file viewer application.
- At 387 files this is by far the largest zone in the web package; periodically audit that internal viewer/server/messaging sub-layers remain directionally correct and don't drift toward each other.
- The mix of server entry points and viewer UI files within one algorithmic zone reflects correct import-graph community detection — production separation is enforced by the build pipeline, not by zone isolation alone.
- Cohesion of 0.98 is the highest in the codebase — this zone is exceptionally well-bounded and its internal modules are tightly related.
- External coupling of 0.02 confirms the viewer application imports very little from outside its own boundary, which aligns with the gateway-only cross-package import rule.
- With 387 files the zone is large enough that sub-layer drift (e.g. viewer components importing directly from server modules) should be verified by boundary-check.test.ts on each refactor.
- Zone "web-viewer-application" has files across 22 directories — consider consolidating under a dedicated directory
- The zone occupies the middle tier of the web package's three-tier stack: mcp-route-layer wires into it from above (3 imports), and it consumes web-shared-foundation below (7 imports). This confirms the zone's role as the application layer, not a composition root.
- Files packages/web/src/landing/landing.ts and its test are algorithmically grouped into monorepo-root despite being a Preact page component inside packages/web/src/ — these should be pinned to web-viewer-application to close the gap in zone coverage.
- web-viewer-application is the middle tier in a documented three-layer stack (mcp-route-layer → web-viewer-application → web-shared-foundation); its bidirectional coupling with mcp-route-layer is bounded by this stack position and should not be treated as a layering violation.
- packages/web/src/landing/landing.ts and packages/web/tests/unit/landing/landing.test.ts are algorithmically placed in monorepo-root but are web viewer UI files; pinning them to web-viewer-application would remove misclassified files from the root zone.
- start.ts is the true server composition root — it wires all route handlers including handleMcpRoute from routes-mcp.ts (mcp-route-layer) — but the Louvain algorithm places it in web-viewer-application because its import neighbors are predominantly the other server files in that zone. This misplacement is the direct source of the 2 upward zone crossings from web-viewer-application into mcp-route-layer; pinning start.ts to mcp-route-layer would eliminate the upward crossings and consolidate all composition-root wiring into one zone.
- The 4 documented internal sub-layers (web-server, web-viewer, viewer-message-pipeline, web-shared) are invisible to zone-level health metrics because Louvain merges all four into web-viewer-application. Internal sub-layer boundary enforcement is therefore entirely dependent on boundary-check.test.ts — a single file that becomes a single point of failure. Any import rule missing from that test accumulates silently until a refactor exposes it.
- The 0.02 external coupling score for web-viewer-application is structurally misleading: it measures only cross-zone edges, not the substantial internal sub-layer coupling across server/viewer/messaging/shared. A zone this large can accumulate significant internal coupling debt while the external metric stays healthy, giving a false sense of architectural cleanliness.
- packages/web/src/server/start.ts is algorithmically placed in web-viewer-application but is the actual server composition root — it imports handleMcpRoute from routes-mcp.ts (mcp-route-layer), creating an upward zone crossing. Pinning start.ts to mcp-route-layer alongside the handler it wires would eliminate the upward import, restore the pure-sink property of the composition-root zone, and make mcp-route-layer's zone membership structurally accurate.
- The boundary-check.test.ts file is the sole enforcement mechanism for the 4 documented internal sub-layer boundaries within web-viewer-application. If a new import path violates a sub-layer rule but is not covered by that test, the violation will accumulate silently — zone metrics will remain green because all violations stay intra-zone. Consider adding explicit import-direction assertions for each sub-layer pair (server→viewer, viewer→messaging, messaging→shared) to make coverage gaps visible.
- With 10 entry points and no single controlled public API surface, there is no safe deprecation workflow for any export: removing or renaming any symbol requires a monorepo-wide search across all 10 consumer locations. A single consolidated re-export barrel (following the convention used by all 5 packages per CLAUDE.md) would make the API surface explicit and safely evolvable.
- rex-gateway.ts lives in web-viewer-application but exists exclusively to serve mcp-route-layer's MCP routing logic. The gateway owner (consumer zone) and the gateway file are in different zones, inverting the expected co-location pattern: the file defining the cross-package boundary lives in the application layer rather than the composition root that uses it.
- The 0.98 cohesion score masks heterogeneous sub-layer membership: server files, viewer UI components, messaging infrastructure, and the composition root (start.ts) all contribute to this single number. Because all four sub-layers are collapsed into one zone, the metric cannot distinguish healthy tight coupling within a sub-layer from unhealthy coupling across sub-layers — a sub-layer drift event would leave the zone cohesion score unchanged.
- rex-gateway.ts (the Rex MCP server factory) lives in web-viewer-application but is consumed exclusively by mcp-route-layer. Moving rex-gateway.ts to mcp-route-layer would co-locate the gateway with its sole consumer, eliminate one cross-zone import, and make the composition-root zone self-contained — a before/after improvement with no source code changes required, only a zone pin update.
- The 10 entry points create an unbounded public API surface with no deprecation path: any symbol exported from any of the 10 entry point files is implicitly public. Consolidating to a single public.ts re-export barrel (matching the convention documented in CLAUDE.md for all 5 packages) would make the API surface explicit, auditable, and safely evolvable without a monorepo-wide grep.
- [call graph] 7235 internal calls, 24 outgoing, 1 incoming (cohesion: 1, coupling: 0)
