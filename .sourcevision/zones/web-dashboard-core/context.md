...
Files: 340, Cohesion: 0.98, Coupling: 0.02
Risk: healthy (score: 0.02)
Description: The primary web application encompassing the PRD viewer, MCP server endpoints, REST API, aggregation layer, and CLI entry point for the unified dashboard.
...
packages/web/src/viewer/components/prd-tree/prd-tree.ts (TypeScript, 797 lines, source)
...
packages/web/src/viewer/styles/prd-tree.css (CSS, 5454 lines, other)
...
packages/web/src/viewer/views/prd.ts (TypeScript, 255 lines, source)
...
Outgoing (this zone → other zones):
  ...
  → prd-input-panels: packages/web/src/viewer/views/domain-sourcevision.ts → packages/web/src/viewer/views/analysis.ts; packages/web/src/viewer/views/rex-dashboard.ts → packages/web/src/viewer/components/prd-tree/smart-add-input.ts
  → task-usage-tracking: packages/web/src/server/routes-hench.ts → packages/web/src/server/incremental-task-usage.ts; packages/web/src/server/start.ts → packages/web/src/server/usage-cleanup-scheduler.ts
  → web-package-scaffold: packages/web/src/viewer/components/active-tasks-panel.ts → packages/web/src/viewer/components/elapsed-time.ts; packages/web/src/viewer/views/domain-rex.ts → packages/web/src/viewer/views/task-audit.ts
...
Incoming (other zones → this zone):
  ...
  ← prd-input-panels: packages/web/src/viewer/components/prd-tree/smart-add-input.ts → packages/web/src/viewer/components/prd-tree/levels.ts; packages/web/src/viewer/views/analysis.ts → packages/web/src/viewer/components/logos.ts
  ← task-usage-tracking: packages/web/src/server/usage-cleanup-scheduler.ts → packages/web/src/server/rex-gateway.ts; packages/web/src/server/usage-cleanup-scheduler.ts → packages/web/src/server/rex-gateway.ts
  ← web-package-scaffold: packages/web/src/viewer/hooks/use-tick.ts → packages/web/src/viewer/polling/index.ts; packages/web/src/viewer/views/task-audit.ts → packages/web/src/viewer/components/logos.ts; packages/web/src/viewer/views/task-audit.ts → packages/web/src/viewer/components/rex-task-link.ts; packages/web/src/viewer/views/task-audit.ts → packages/web/src/viewer/types.ts

</imports>

<findings>

[observation] [info] Contains 56% of project files (340/605) — subdivided into 8 sub-zones
[observation] [info] High cohesion (0.98) — files are tightly interconnected
[observation] [warning] Bidirectional import relationship with both 'panel' and 'web-server' zones warrants attention — mutual imports between zones can make refactoring difficult over time.
[observation] [info] Cohesion of 0.98 across 340 files is unusually high; confirms strong internal consistency but also suggests this zone may be absorbing too many responsibilities that should be separated.
[observation] [info] Coupling of 0.02 demonstrates excellent boundary discipline for a zone of this size, likely enforced by the gateway module pattern described in CLAUDE.md.
[suggestion] [info] Zone "web-dashboard-core" has files across 22 directories — consider consolidating under a dedicated directory
[suggestion] [info] Document the implicit convention that intra-package zone boundaries are convention-only (except server/viewer, which has an executable guard in boundary-check.test.ts); add a CONTRIBUTING note or ADR explaining which boundaries are tested and which rely on review discipline, so new contributors do not inadvertently create unguarded cross-zone imports.
[suggestion] [info] The zone name 'web-dashboard-core' does not signal its dual server+viewer nature, which is the root cause of all bidirectional coupling findings. Consider renaming to 'web-runtime' and pairing the rename with the eventual zone split to make the transition name a deliberate waypoint rather than a permanent label.
[relationship] [critical] web-viewer imports from web-unit 6 times — the largest single cross-zone edge in the graph. If web-unit is a test-support zone, this is a critical production→test boundary violation.
[relationship] [warning] web-viewer → web (2 imports) pulls scaffold or config-layer code into the viewer at runtime; confirm these are not test utilities or startup-only helpers leaking into the production module graph.
[anti-pattern] [warning] physics.ts is classified as part of the 'web-unit' sub-zone due to Louvain co-placement with its test files, but it is a pure production module (Barnes-Hut quad-tree engine extracted from graph.ts). This misclassification generates a false 'production-to-test' coupling alarm; relocate physics.ts adjacent to graph.ts in the viewer/graph/ directory to align its zone membership with its actual consumers.
...
- Contains 56% of project files (340/605) — subdivided into 8 sub-zones
- At 340 files with 0.98 cohesion, this zone is architecturally coherent but very large — consider whether sub-domains like the MCP server, REST handlers, and viewer components could be split into distinct zones as the package grows.
- The 0.02 coupling score is exceptional and indicates strong encapsulation relative to other packages in the monorepo.
- Bidirectional imports exist between this zone and both 'panel' and 'web-server' — the web-viewer imports from them and they import back, which is worth monitoring to avoid tight circular dependencies.
- Cohesion of 0.98 across 340 files is unusually high; confirms strong internal consistency but also suggests this zone may be absorbing too many responsibilities that should be separated.
- Bidirectional import relationship with both 'panel' and 'web-server' zones warrants attention — mutual imports between zones can make refactoring difficult over time.
- Coupling of 0.02 demonstrates excellent boundary discipline for a zone of this size, likely enforced by the gateway module pattern described in CLAUDE.md.
- Zone "web-dashboard-core" has files across 22 directories — consider consolidating under a dedicated directory
- The web-viewer → web-unit edge (6 imports) is the highest single cross-zone import count in the graph; if web-unit represents a test-utilities zone, this is a production-to-test coupling violation that must be resolved — test helpers must never be imported by production modules.
- web-viewer → web: 2 imports means production viewer code depends on the package scaffold/config zone at runtime; scaffold-level code should typically be a startup dependency only, not a viewer import, and warrants an audit to confirm what is being pulled in.
- web-viewer imports from web-unit 6 times — the largest single cross-zone edge in the graph. If web-unit is a test-support zone, this is a critical production→test boundary violation.
- web-viewer → web (2 imports) pulls scaffold or config-layer code into the viewer at runtime; confirm these are not test utilities or startup-only helpers leaking into the production module graph.
- The 'web-unit' sub-zone (id: web-viewer/web-unit) contains physics.ts — a Barnes-Hut quad-tree force-directed graph physics engine — grouped by Louvain with its two graph test files. The zone name is purely an artifact of Louvain topology labeling. The 6 cross-zone imports web-viewer → web-unit are production-to-production imports of physics.ts, definitively not a production-to-test boundary violation. All prior alarms about this edge are false positives and should be retired.
- The physics.ts module (currently co-grouped with test files in the web-unit sub-zone) is the sole production reason for the web-viewer → web-unit edge; moving physics.ts physically closer to graph.ts (the file it was extracted from, per its own header comment) would eliminate the cross-zone edge and resolve the misleading naming simultaneously.
- web-dashboard-core spans both src/server/ and src/viewer/ subtrees within the same 340-file zone — the boundary-check.test.ts explicitly tests that these two subtrees do not import from each other, confirming the team recognizes a logical boundary that the zone graph does not reflect. The existence of this guard test implies the zone should eventually be split into server and viewer sub-zones.
- physics.ts is classified as part of the 'web-unit' sub-zone due to Louvain co-placement with its test files, but it is a pure production module (Barnes-Hut quad-tree engine extracted from graph.ts). This misclassification generates a false 'production-to-test' coupling alarm; relocate physics.ts adjacent to graph.ts in the viewer/graph/ directory to align its zone membership with its actual consumers.
- The zone name 'web-dashboard-core' implies a single coherent core, but boundary-check.test.ts (in websocket-infrastructure) exists precisely to enforce that src/server/ and src/viewer/ within this zone do not import each other — the zone name actively obscures the server/viewer split that the team's own test infrastructure acknowledges. A name like 'web-runtime' would be more honest about its dual-context nature.
- No intra-package zone boundary is enforced by an executable test other than the server/viewer split in boundary-check.test.ts; all other zone-pair boundaries (web-viewer↔panel, web-viewer↔task-usage-tracking) are convention-only with no guard test, making the server/viewer boundary the only contractually enforced boundary in the entire web package.
- Document the implicit convention that intra-package zone boundaries are convention-only (except server/viewer, which has an executable guard in boundary-check.test.ts); add a CONTRIBUTING note or ADR explaining which boundaries are tested and which rely on review discipline, so new contributors do not inadvertently create unguarded cross-zone imports.
- The zone name 'web-dashboard-core' does not signal its dual server+viewer nature, which is the root cause of all bidirectional coupling findings. Consider renaming to 'web-runtime' and pairing the rename with the eventual zone split to make the transition name a deliberate waypoint rather than a permanent label.
- [call graph] 6245 internal calls, 23 outgoing, 6 incoming (cohesion: 1, coupling: 0)
...
Cross-dependencies between sub-zones:
  ...
  web-viewer/route → web-viewer/web: 5
  web-viewer/route → web-viewer/web-3: 1
  web-viewer/web → web-viewer/crash: 3
  web-viewer/web → web-viewer/route: 1
...
- **Web Viewer/crash** (`web-viewer/crash`): 3 files, cohesion 0.33, coupling 0.67
- **Web Viewer/route** (`web-viewer/route`): 4 files, cohesion 0.57, coupling 0.43
- **Web Viewer/web** (`web-viewer/web`): 272 files, cohesion 0.97, coupling 0.03
  - Has 7 nested sub-zone(s)
- **Web Viewer/web 2** (`web-viewer/web-2`): 43 files, cohesion 1, coupling 0
- **Web Viewer/web 3** (`web-viewer/web-3`): 3 files, cohesion 0.36, coupling 0.64
...
- **Web Viewer/web Viewer** (`web-viewer/web-viewer`): 9 files, cohesion 0.68, coupling 0.32
