# Command Validation — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Command Validation (`command-validation`)
Files: 10, Cohesion: 0.90, Coupling: 0.10
Description: Guard system that validates and enforces constraints on agent commands before execution.
...
Outgoing (this zone → other zones):
  → agent-core: packages/hench/tests/unit/guard/guard-integration.test.ts → packages/hench/src/schema/v1.ts; packages/hench/tests/unit/tools/shell.test.ts → packages/hench/src/schema/v1.ts; packages/hench/tests/unit/tools/shell.test.ts → packages/hench/src/tools/shell.ts

Incoming (other zones → this zone):
  ← agent-core: packages/hench/src/agent/lifecycle/loop.ts → packages/hench/src/guard/index.ts; packages/hench/src/tools/files.ts → packages/hench/src/guard/paths.ts
...
[suggestion] [info] contracts.ts pattern should be adopted by other service zones (execution-queue) for consistent interface definition
