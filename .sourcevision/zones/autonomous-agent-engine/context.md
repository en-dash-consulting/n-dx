...
Files: 82, Cohesion: 1.00, Coupling: 0.00
Description: Autonomous task execution engine that picks Rex tasks, builds briefs, orchestrates Claude API tool-use loops, and maintains run history with safety guardrails.
Lines: 14669
...
packages/hench/src/cli/commands/run.ts (TypeScript, 593 lines, source)
...
packages/hench/src/prd/ops.ts (TypeScript, 47 lines, source)
packages/hench/src/public.ts (TypeScript, 66 lines, source)
...
Internal:
  ...
  packages/hench/src/agent/planning/brief.ts → packages/hench/src/prd/ops.ts {findNextTask, findActionableTasks, collectCompletedIds, findItem, walkTree}
  ...
  packages/hench/src/tools/rex.ts → packages/hench/src/prd/ops.ts {computeTimestampUpdates, findAutoCompletions}
...
- Contains controlled orchestration coupling with Rex for legitimate task management operations
- Implements comprehensive agent lifecycle management with token budgets and retry logic
- Missing public API pattern found in other packages - lacks public.ts and exports field
- 14 direct imports from rex package create extensive but controlled runtime dependency for task execution
- Single runtime import gateway (ops.ts) controls all cross-package coupling — makes the hench→rex dependency surface explicit
- Gateway pattern implementation (ops.ts) successfully consolidates 8 rex imports into a single controlled coupling point
- Missing public API pattern found in all other packages — lacks complete public.ts exports field with comprehensive external interface
- Package.json exports completeness achieved — hench now properly exports public.ts through package.json main field, completing the standardization
- Runtime function exclusion is intentional design — public.ts deliberately excludes agent loops and tool dispatch to enforce CLI-only consumption
...
- **Tests** (`hench/tests`): 31 files, cohesion 0.78, coupling 0.22
- **Tests 2** (`hench/tests-2`): 7 files, cohesion 0.67, coupling 0.33
