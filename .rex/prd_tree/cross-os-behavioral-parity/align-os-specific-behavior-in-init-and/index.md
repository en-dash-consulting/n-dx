...
| Title | Status |
|-------|--------|
| [Confirm AC 7 of the freeze-verify-kill task: a POSIX CI run must actually execute the freeze policy green](./confirm-ac-7-of-the-freeze-verify-kill.md) | completed |
| [Decide and document the Windows available-memory strategy for hench admission](./decide-and-document-the-windows.md) | completed |
| [Enforce API-key file permissions on Windows via ACLs, or stop claiming they are set](./enforce-api-key-file-permissions-on.md) | completed |
| [Freeze-verify-kill: make the POSIX tree kill definitive for timeouts](./freeze-verify-kill-make-the-posix-tree.md) | completed |
| [Guard usePanZoom against a zero-sized element, and give the hook its first test](./guard-usepanzoom-against-a-zero-sized.md) | completed |
| [hench's RunChangeDetector shares the mtime-granularity blind spot](./hench-s-runchangedetector-shares-the.md) | completed |
| [hench's scoped post-task test run always fails on Windows (backslash paths eaten by sh -c)](./hench-s-scoped-post-task-test-run.md) | completed |
| [pair-programming.js times out with bare SIGTERM — three sites, no tree kill, no escalation, no await](./pair-programming-js-times-out-with.md) | completed |
| [Route ndx start stop through the unified terminateTree contract](./route-ndx-start-stop-through-the.md) | completed |
| [Spawn exec's children through spawn so POSIX can create a real process group](./spawn-exec-s-children-through-spawn-so.md) | completed |
| [Tree-kill on shell-command timeout: a timed-out command keeps running and keeps writing](./tree-kill-on-shell-command-timeout-a.md) | completed |
