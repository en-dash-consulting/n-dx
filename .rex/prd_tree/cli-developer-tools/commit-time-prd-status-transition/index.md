...
| Title | Status |
|-------|--------|
| [Add structured PRD status transition trailer to commit messages](./add-structured-prd-status-transition.md) | completed |
| [Stage PRD status transition write into the same commit that completes the task](./stage-prd-status-transition-write-into.md) | completed |
