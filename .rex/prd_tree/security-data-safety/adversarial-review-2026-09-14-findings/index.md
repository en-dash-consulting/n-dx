...
| [`ndx ci` config-secrets step never fails on a git-tracked `.n-dx.local.json`](./ndx-ci-config-secrets-step-never-fails.md) | completed |
...
| [`ndx export` silently appends its out-dir to `.gitignore`, even when that directory is git-tracked](./ndx-export-silently-appends-its-out.md) | completed |
...
| [`readBody` destroys an over-cap chunked request before any response, so the client sees EPIPE instead of the documented 400](./readbody-destroys-an-over-cap-chunked.md) | completed |
