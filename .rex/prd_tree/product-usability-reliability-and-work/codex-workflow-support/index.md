...
| Title | Status |
|-------|--------|
| [Make Codex quota/token retrieval work for codex login (session auth) users](./make-codex-quota-token-retrieval-work.md) | completed |
| [Reconcile Codex model-id catalogs and fix dead light-tier alias](./reconcile-codex-model-id-catalogs-and.md) | completed |
| [Wire Codex token accounting into event-pipeline close path](./wire-codex-token-accounting-into-event.md) | completed |
