...
tags:
  ...
  - "severity:medium"
source: "ndx-capture"
startedAt: "2026-08-31T23:24:12.780Z"
completedAt: "2026-08-31T23:38:06.270Z"
endedAt: "2026-08-31T23:38:06.270Z"
resolutionType: "code-change"
resolutionDetail: "Commit 9a58c228. (1) Reproduced deliberately: concurrent `ndx config hench` spawns under busy-spin CPU load, with the update-check cache deleted per round to force the network path — 390 aborts in 448 spawns (87%), exit 0xC0000409. A warm cache hides it entirely, which is why the original sighting was a one-off. (2) Handle identified: the update check, via the CLI's own --quiet bisect (0/448 with the check skipped, same load). Specifically the DNS lookup its fetch queues on libuv's threadpool, completing into a loop that process.exit() had already begun closing — hence uv_async_send on a closing handle at src/win/async.c:94. The other two candidates were excluded: dispose() only removes JS listeners, and `ndx config` spawns no children. (3) The 500ms race timer is now cleared in a finally block, so no timer outlives the race. (4) Verified: 0 aborts in 640 spawns under the same load; ordinary exits keep correct codes with no added latency. (5) Teardown correction, not a retry or mute — flushAndExit now sets process.exitCode and lets the loop drain, with an unref'd 2s fallback preserving the never-hang guarantee, and startUpdateCheck takes an AbortSignal so the check is stopped rather than merely un-awaited. Worth recording: aborting alone was NOT enough (385/448) because uv_getaddrinfo cannot be cancelled once queued; the process.exit() removal is what fixed it."
acceptanceCriteria:
...
lastModified: "2026-08-31T23:38:06.298Z"
