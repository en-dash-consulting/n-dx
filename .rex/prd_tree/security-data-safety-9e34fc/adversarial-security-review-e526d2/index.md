...
startedAt: "2026-09-11T21:40:08.120Z"
completedAt: "2026-09-11T21:40:08.120Z"
endedAt: "2026-09-11T21:40:08.120Z"
acceptanceCriteria: []
...
lastModified: "2026-09-11T21:40:08.126Z"
...
| Title | Status |
|-------|--------|
| [`/api/workflow/apply-suggestion` writes arbitrary dotted keys into `.hench/config.json`](./api-workflow-apply-suggestion-d8d1d0.md) | completed |
| [Gemini provider sends the API key in the URL query string](./gemini-provider-sends-the-api-99467c.md) | completed |
| [hench command guard passes newline-separated commands and `>` redirection](./hench-command-guard-passes-6f9f95.md) | completed |
| [hench `git` tool re-joins args into `sh -c` with no shell-operator check](./hench-git-tool-re-joins-args-0ce3eb.md) | completed |
| [`.n-dx.json` receives API keys but `ndx init` never gitignores it](./n-dx-json-receives-api-keys-but-76c5c5.md) | completed |
| [`ndx export` / `--deploy=github` publish full hench transcripts with no redaction or confirmation](./ndx-export-deploy-github-208b96.md) | completed |
| [Web server `readBody` has no request-size cap](./web-server-readbody-has-no-f80187.md) | completed |
| [WebSocket upgrade accepts any Origin — dashboard broadcasts readable cross-site](./websocket-upgrade-accepts-any-04a041.md) | completed |
