...
startedAt: "2026-09-17T03:27:58.878Z"
completedAt: "2026-09-17T03:27:58.878Z"
endedAt: "2026-09-17T03:27:58.878Z"
acceptanceCriteria: []
...
lastModified: "2026-09-17T03:27:59.249Z"
...
| Title | Status |
|-------|--------|
| [Dashboard config validator accepts values hench's schema rejects, leaving `.hench/config.json` unloadable](./dashboard-config-validator-0a85ae.md) | completed |
| [hench command guard passes `$(…)` and backticks inside double quotes, which `sh` expands](./hench-command-guard-passes-and-bd5560.md) | completed |
| [hench command guard's POSIX quote model lets `&`, `|`, `<`, `>` through on the cmd.exe fallback](./hench-command-guard-s-posix-26913c.md) | completed |
| [`ndx ci` config-secrets step never fails on a git-tracked `.n-dx.local.json`](./ndx-ci-config-secrets-step-b53574.md) | completed |
| [`ndx export` sanitizer leaves test output, command lines, and audit errors in published run records](./ndx-export-sanitizer-leaves-dfe510.md) | completed |
| [`ndx export` silently appends its out-dir to `.gitignore`, even when that directory is git-tracked](./ndx-export-silently-appends-its-1d91dd.md) | completed |
| [`ndx export` strips `error` from per-run files but still publishes it in the `runs.json` index](./ndx-export-strips-error-from-3d15ab.md) | completed |
| [`readBody` destroys an over-cap chunked request before any response, so the client sees EPIPE instead of the documented 400](./readbody-destroys-an-over-cap-88f193.md) | completed |
