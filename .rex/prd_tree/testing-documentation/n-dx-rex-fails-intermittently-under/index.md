...
completedAt: "2026-09-11T22:40:48.172Z"
endedAt: "2026-09-11T22:40:48.172Z"
resolutionType: "code-change"
resolutionDetail: "Root cause: transient empty-lock-file race in file-lock.ts isLockStale — O_EXCL creates the lock file before its JSON is written, and a concurrent reader treated the empty window as \"malformed = stale\", unlinking a live writer's lock and interleaving two import-bundle writes (caught by the stale-save guard). Fixed by treating empty/unparseable lock content as not-stale (wait/timeout, never steal). The description's STALE_LOCK_MS suspect was already gone (39bff349) and was not the cause. 2 regression tests; 60/60 concurrency runs clean post-fix (was ~7% fail), full rex suite + full pnpm test green. Also shipped core/gitignore.js in package.json files (packaging regression from this session's earlier work, surfaced by the full-suite run)."
acceptanceCriteria:
...
lastModified: "2026-09-11T22:40:48.179Z"
