...
startedAt: "2026-09-25T04:27:17.913Z"
completedAt: "2026-09-25T04:50:55.293Z"
endedAt: "2026-09-25T04:50:55.293Z"
resolutionType: "code-change"
resolutionDetail: "Wrapped mount/re-render/unmount calls in preact/test-utils act() across 6 viewer hook test files (use-polling-suspension, use-project-status, use-git-status, use-active-operations, use-cli-name, use-pan-zoom), so Preact's after-paint effect queue is flushed synchronously via act()'s options.requestAnimationFrame patch instead of the real rAF+setTimeout fallback that can outlive jsdom teardown. Fixed one exposed race in use-git-status's \"fetches status once on mount\" test (was asserting on fetch-called instead of the resolved state). Audited all use-*.test.ts viewer hook tests; use-crash-recovery, use-dom-performance-monitor, use-facet-state, use-live-tick already used act() correctly; use-hub-queue has no rendering. Full @n-dx/web suite (274 files/4034 tests) and full monorepo preflight pass."
acceptanceCriteria:
...
lastModified: "2026-09-25T04:50:57.988Z"
