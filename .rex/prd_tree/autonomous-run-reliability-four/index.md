...
| [Codex file_change event shape is unverified, so the livelock progress fix can silently not work](./codex-file-change-event-shape-is.md) | completed |
...
| [Publish only a complete PRD snapshot after a retry](./publish-only-a-complete-prd-snapshot.md) | completed |
...
| [Retry lock acquisition when a contending lock vanishes during inspection](./retry-lock-acquisition-when-a.md) | completed |
...
| [Task selection must use the shared completed-child predicate](./task-selection-must-use-the-shared.md) | completed |
