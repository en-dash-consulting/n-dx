---
id: "aa92ccd9-989b-4296-8935-c83c771f34d0"
level: "feature"
title: "Token Usage Attribution per PRD Item"
status: "completed"
source: "smart-add"
startedAt: "2026-04-23T16:02:11.657Z"
completedAt: "2026-04-23T16:02:11.657Z"
description: "Capture and aggregate LLM token usage (input, output, cached, total) against the PRD item each hench run is working on, then roll those totals up the tree so tasks, features, and epics each expose their own usage."
---

## Children

| Title | Status |
|-------|--------|
| [Attribute hench run token usage to the active PRD task](./attribute-hench-run-token-usage-to-the-34344b18/index.md) | completed |
| [Roll up token usage from subtasks to tasks, features, and epics](./roll-up-token-usage-from-subtasks-to-61022482/index.md) | completed |
