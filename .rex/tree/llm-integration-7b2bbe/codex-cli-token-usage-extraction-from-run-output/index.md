---
id: "508bf0a8-88fb-4c17-a183-d5460b066617"
level: "feature"
title: "Codex CLI Token Usage Extraction from Run Output"
status: "completed"
source: "smart-add"
startedAt: "2026-04-14T20:10:12.593Z"
completedAt: "2026-04-14T20:10:12.593Z"
acceptanceCriteria: []
description: "Codex CLI prints token usage statistics to stdout after each invocation. This feature captures those printed counts and feeds them into hench's token tracking system so Codex runs appear in usage analytics alongside Claude runs."
---

## Children

| Title | Status |
|-------|--------|
| [Implement and integrate Codex CLI token usage extraction into hench run tracking](./implement-and-integrate-codex-cli-token-usage-f73fe7/index.md) | completed |
