---
id: "a32f8a34-2c34-4ea1-9652-d85d0ba4e8d2"
level: "feature"
title: "PRD Merge Context Graph View"
status: "completed"
source: "smart-add"
startedAt: "2026-04-24T19:59:27.566Z"
completedAt: "2026-04-24T19:59:27.566Z"
acceptanceCriteria: []
description: "Interactive visual graph in the dashboard that shows the PRD tree as a context graph overlaid with git merge history, so users can see which PRD items correspond to which merges and what code changes were introduced in each merge."
---

## Children

| Title | Status |
|-------|--------|
| [Build merge history data pipeline linking git merges to PRD items](./build-merge-history-data-pipeline-linking-git-3b7bdd/index.md) | completed |
| [Render interactive PRD-and-merge context graph in the dashboard](./render-interactive-prd-and-merge-context-graph-in-72855b/index.md) | completed |
