---
id: "be363df5-da5d-42a8-b5fa-6bfaf294982d"
level: "feature"
title: "Consolidate PRD Files into Single 'prd' File"
status: "completed"
source: "smart-add"
startedAt: "2026-04-23T02:30:34.820Z"
completedAt: "2026-04-23T02:30:34.820Z"
description: "Unify the branch-scoped multi-file PRD storage back into a single canonical file named 'prd' for both newly generated and existing PRDs, including migration of any existing split files."
---

## Children

| Title | Status |
|-------|--------|
| [Merge branch-scoped PRD files into single canonical 'prd' file](./merge-branch-scoped-prd-files-into-37c43e3c/index.md) | completed |
| [Update PRD storage tests for single-file 'prd' location](./update-prd-storage-tests-for-single-54be143d/index.md) | completed |
