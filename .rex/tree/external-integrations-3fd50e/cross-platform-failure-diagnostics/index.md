---
id: "4ee969d6-cb9e-41c1-85ac-f79b3b6a6ac3"
level: "feature"
title: "Cross-Platform Failure Diagnostics"
status: "completed"
source: "smart-add"
startedAt: "2026-04-07T22:58:12.823Z"
completedAt: "2026-04-07T22:58:12.823Z"
acceptanceCriteria: []
description: "Refocus smoke parity validation on standardized error semantics so failures remain easy to compare, diagnose, and fix regardless of operating system."
---

## Children

| Title | Status |
|-------|--------|
| [Document error code meanings and remediation expectations](./document-error-code-meanings-and-remediation-expectations/index.md) | completed |
| [Refactor smoke parity checks to compare normalized error codes](./refactor-smoke-parity-checks-to-compare-normalized-0886fe/index.md) | completed |
