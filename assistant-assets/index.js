 ...
 *
 * The generated file explains the n-dx workflow, lists available skills,
 * and describes the Rex and SourceVision MCP capabilities so Codex can
 * participate in the n-dx workflow without separate onboarding.
 *
 * All content is derived from the manifest — skill metadata, MCP server
 * descriptors, and tool classifications — so the file stays in sync with
 * the canonical definitions automatically.
 *
...
export function renderAgentsMd() {
  ...
  sections.push(
    ...
    "# n-dx",
    "",
    "AI-powered development toolkit. Three packages that chain together: " +
      "analyze a codebase (sourcevision), build a PRD (rex), execute tasks " +
      "autonomously (hench).",
    "",
  );
