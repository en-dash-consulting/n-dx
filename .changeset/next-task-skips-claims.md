...
`GET /api/rex/next` and the dashboard's Up Next card suggested the
...
claimed by <worktree> — showing the next unclaimed task`. `GET
/api/rex/claims` and the PRD tree's claim chips are unchanged.
