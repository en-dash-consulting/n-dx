...
New `ndx claim` (`rex claim`): `list` shows every live claim with task title, worktree, holder liveness, state and expiry; `release <taskId>` frees one — from whichever worktree the operator is standing in when the claim is held or its holder is dead — refusing while the holder is alive unless `--force`; `release --all` frees this worktree's claims. `--format=json` throughout.
