...
| `.n-dx.json` | ✅ | Project config overrides (vendor, model, dashboard port, zone pins). `ndx config` never writes API keys or machine paths here — those go to `.n-dx.local.json`. `ndx ci` fails if a tracked `.n-dx.json` contains an `api_key` |
| `ndx-export/` | ❌ | Default `ndx export` output — a second copy of the PRD and run summaries; `ndx init` and `ndx export` both add it to `.gitignore` |
