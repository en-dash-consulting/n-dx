...
- **Recorded at** — 2026-09-22T17:19:00.439Z
- **Commit** — `641c6b762167`
- **Content hash** — `f6bff532909b348d` (identifies the measurement itself; `tests/e2e/prompt-census.test.js` fails when the repo no longer matches it)
- **Model for cost/context figures** — `claude-sonnet-5`
- **Surfaces** — 36
- **Per-call total** — 22,003 tokens (what every surface costs, summed)
- **Unique fixed text** — 13,516 tokens (distinct text a rewrite has to edit)
...
## hench — 3,846 per-call / 3,846 unique, 9 surfaces
...
| `buildReviewBrief` | `packages/hench/src/agent/analysis/adversarial-review.ts` | Brief handed to the reviewer — what to attack and where to report. | 140 | 1,653 | — | 1,653 |
