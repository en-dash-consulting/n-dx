...
| `ndx prd export\|import` | Carry the PRD between machines as a portable JSON bundle (`--out`, `--in`, `--replace`), or write a stakeholder document with `--format=narrative` (`--item`, `--include-completed`; one-way) — distinct from `ndx export` above |
