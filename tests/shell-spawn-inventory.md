...
| `packages/hench/tests/integration/test-gate-kill-tree.test.ts` | Runs `node suite.cjs` through `runTestGate` — shell-syntax-free, so it launches identically through `sh` and cmd.exe, as in `test-gate-timeout-config.test.ts`. The real-process cases are `describe.skipIf(win32)` anyway, because a libuv job object reaps node trees there regardless of the kill under test; the `describeTermination` cases spawn nothing |
| `packages/hench/tests/unit/tools/test-gate-failure-digest.test.ts` | Spawns no shell: every `runTestGate` and `finalizeRun` case stubs `execShellCmd`, and `extractFailureDigest` is pure. The git fixture uses `execFileSync("git", …)` directly |
