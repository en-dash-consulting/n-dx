...
const BOUNDARY_FILES = [
  ...
  },
  {
    ...
    maxExports: 72,
    description: "web→rex gateway (raised from 69 to 72 for estimateCostFromTotals plus its CostEstimate and ModelCostLine types: the dashboard aggregates its own four-package token shape but must quote the same dollar figure as ndx usage for the same runs, so it imports rex's per-model pricing arithmetic rather than keeping the local copy that priced every token at one fallback rate — tests/unit/token-pricing-parity.test.js pins that routes-token-usage.ts has no pricing loop of its own. Raised from 68 to 69 for checkTreeConformance: the Execute route refuses to start a run against a PRD tree this build would re-slug, and the tree's own definition of conformance — the slug rule plus the tree-meta marker — is rex's to hold; a web-local copy would be a second definition of what a conformant tree is, free to drift from the one the store enforces at write time. Raised from 66 to 68 for openClaimsStore and resolveClaimHolder, 0.6.0 PR 6: the execute route answers 409 naming the worktree that holds a task, and the claims file lives in the git common dir that only rex's store knows how to find; domain types, MCP server factory, tree utilities, token + duration rollup, constants, Markdown serializer/parser, folder-tree parser/slug resolver, legacy PRD migration, PRD tree backup snapshots for the dashboard's Restore panel — isValidSnapshotId added to reject a path-traversal id in POST /api/rex/restore before it reaches restoreFromBackup's fs.rm)",
  },
  {
  ...
  },
  {
    ...
    maxExports: 42,
    description: "hench→rex gateway (schema, store, tree, task selection, timestamps). Raised from 40 to 42 for takeSaveFileReport and its SaveFileReport type (0.7.1 PR C): hench's completion and --reset-deferred commits stage exactly the files the PRD save wrote and deleted instead of the whole `.rex/prd_tree/` — which once swept a 1,378-file in-flight rename into a 'task completed' commit — and only rex's serializer knows which files a save touched, so the list has to cross the boundary rather than be re-derived by a hench-side scan that would drift from the serializer's actual writes. Raised from 39 to 40 for ClaimHoldReason (0.7.1 PR D): the completion gate holds a claim rather than releasing it when it refuses on uncommitted work, and the set of reasons a claim can be held is rex's to define — the store's liveness rule keys off it, so a hench-local copy would be a second definition of what makes a claim outlive its pid. Raised from 38 to 39 for checkTreeConformance: an autonomous run writes the PRD when it completes, so a run started against a tree this build would re-slug turns a whole-tree rewrite into a 'task completed' commit inside whatever branch is open. The gate has to ask rex what a conformant tree is — the same question rex validate and the store's write guard ask — because a hench-local copy would be a second definition free to drift from them. Raised from 37 to 38 for SelectionReasonCode: hench's task header used to decide what to print by substring-matching explainSelection's rendered sentence, so rewording that sentence silently blanked the finalize label. The discriminator is rex's to define — a hench-local copy of the three codes would be a second definition of why a task was picked, free to drift from the one that sets it. Raised from 32 to 37 for the cross-worktree claims surface (0.6.0 PR 6): openClaimsStore and resolveClaimHolder plus the ClaimsStore, ClaimHolder and TaskClaim types — the run claims the task it selects so other worktrees pass over it, and the store lives in the git common dir that only rex knows how to locate; a hench copy would be a second definition of where claims live. Raised from 30 to carry TREE_META_FILENAME: `.rex/tree-meta.json` is rewritten by every folder-tree save, so hench has to stage and discount it wherever it stages and discounts the tree, and hardcoding the name at those three sites is exactly the drift that left it in nobody's list and refused every task completion. Raised from 31 to carry findParentResets: withdrawing a completion claim has to reopen the ancestors the same run's cascade closed, and rex already owns that computation — reimplementing the ancestor walk in hench would be a second definition of which parents are inconsistent, free to drift from the add path's.",
  },
  {
    file: "packages/hench/src/prd/llm-gateway.ts",
    maxExports: 174,
    description: "hench→llm-client gateway (config, constants, JSON, output, errors, exec, runtime-contract, codex-policy, diagnostics, tool-schema, provider-registry, vendor-error-classification, failover, color/model helpers, token accumulation, and model catalogs). The combined branch exposes both terminateProcessTree for complete Windows CLI-tree cleanup and resolveLocalTimeoutMs for the local-provider timeout configuration; both are consumed only through this gateway. Raised from 166 to 168 for listWorktrees and its GitWorktree type (0.6.0 PR 5, worktree awareness): the async sibling of getWorktreeRoot, so hench's cross-worktree run views read the same realpath-resolved worktree list the dashboard does rather than shelling out to git themselves. Raised from 168 to 170 for hasPosixShell and resolveShellKind: the command guard must know whether run_command will be interpreted by sh or cmd.exe, and that decision has to come from the same function execShellCmd uses (a copy in hench would drift and re-open the cmd.exe bypass). Raised from 170 to 174 for createProgressReporter, setActiveProgressReporter, getActiveProgressReporter and the ProgressReporter type (0.7.1 trust-copy, monotonic CLI progress): hench's run loop registers the same reporter the LLM providers' default rate-limit retry handler prints through, so the reporter and its registry have to be the one llm-client defines rather than a hench-local copy that could disagree with what a provider's retry line just interrupted. Note the hench-side turn-banner routing is currently a guard rather than a live fix — agentLoop is entered once per task, so the printed number is unchanged today; if that routing is ever removed, drop this cap back to 170 with it.",
  },
];
