...
describe("renderCodexConfigToml", () => {
  ...
  it("each server section has command = \"<cliName>\"", () => {
    const content = renderCodexConfigToml("ndx");
    const sections = content.split("[mcp_servers.").slice(1);
    for (const section of sections) {
      expect(section).toContain('command = "ndx"');
    }
  });

  it("each server section has args with cliCommand, mcp command, and cwd-relative '.'", () => {
    const content = renderCodexConfigToml("ndx");
  ...
  it("escapes backslashes and quotes in the cli name", () => {
    const content = renderCodexConfigToml('weird"\\name');
    // Backslash and quote should be escaped in the TOML string
    expect(content).toContain('command = "weird\\"\\\\name"');
  });
});
...
describe("setupCodexIntegration", () => {
  ...
  it("config.toml has no absolute paths — tmpDir never appears", () => {
    setupCodexIntegration(tmpDir);
    const content = readFileSync(join(tmpDir, ".codex", "config.toml"), "utf-8");
